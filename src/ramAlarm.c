...
// 공유 메모리에서 RAM 사용량 모니터링
void monitor_ram_usage(int shmid, double threshold, int is_max)
{
    while (1)
    {
        // 공유 메모리 연결
        ...
        if ((is_max && usage > threshold) || (!is_max && usage < threshold))
        {
            ...
            char message[100];
            snprintf(message, sizeof(message), "현재 RAM 사용량: %.2f%%", usage);
            show_notification(
                is_max ? "⚠️ RAM 사용량 초과 경고" : "⚠️ RAM 낮은 사용량 경고",
                message);
...
int main(int argc, char *argv[])
{
    ...
    // 공유 메모리 연결
    key_t key = ftok("ram_usage", 66); // RAM 사용량용 키
    ...
    monitor_ram_usage(shmid, threshold, is_max);
